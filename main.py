...
# =====================================================================
# --- ⚔️ ระบบอัปเดตอาวุธแก๊ง Dark Monday (WEAPON SYSTEM MODULE) ---
# =====================================================================

def load_weapons(guild_id):
    return load_data(guild_id, "weapons", {})
...
class WeaponBoardPersistentView(discord.ui.View):
    ...
    @discord.ui.button(label="🔄 อัปเดตอาวุธของฉัน", style=discord.ButtonStyle.primary, custom_id="btn_weapon_update_my_gear")
    async def update_my_gear(self, button: discord.ui.Button, interaction: discord.Interaction):
        gid = str(interaction.guild.id)
        weapons_data = load_weapons(gid)
        user_slot = None
        for s_key, s_val in weapons_data.items():
            if s_val.get("user_id") == str(interaction.user.id):
                user_slot = s_key
                break
                
        if not user_slot:
            msg = await interaction.response.send_message("⚠️ **คุณยังไม่มีรายชื่ออยู่ในสล็อต 01-30 ของแก๊ง!**\nโปรดติดต่อแอดมินเพื่อเพิ่มชื่อเข้าสล็อตก่อนอัปเดตอาวุธครับ", ephemeral=True)
    ...
    @discord.ui.button(label="⚙️ จัดการสล็อต/รายชื่อ", style=discord.ButtonStyle.danger, custom_id="btn_weapon_admin_manage")
    async def admin_manage(self, button: discord.ui.Button, interaction: discord.Interaction):
        is_admin = interaction.user.guild_permissions.administrator or interaction.user.guild_permissions.manage_guild or any(r.name in ["Admin", "ผู้ดูแล"] for r in interaction.user.roles)
        if not is_admin:
            msg = await interaction.response.send_message("⚠️ **ไม่สามารถใช้งานได้!**\nเฉพาะผู้ดูแลระบบ (Admin) เท่านั้นที่สามารถจัดการสล็อตอาวุธได้ครับ", ephemeral=True)
            await asyncio.sleep(5)
            try: await msg.delete()
            except: pass
            return
...
# 🟢 [แก้ไขจุดที่ 1] สร้างตัวเลือกเลเวลแยกเฉพาะแต่ละชนิดอาวุธ เพื่อให้แสดงชื่อและไอคอนหน้ากล่องชัดเจน
KNIFE_OPTIONS = [
    discord.SelectOption(label="❌ มีด : ไม่มีอาวุธ", value="-"),
    discord.SelectOption(label="🔪 มีด : เลเวล +0", value="+0"),
    discord.SelectOption(label="🔪 มีด : เลเวล +1", value="+1"),
    discord.SelectOption(label="🔪 มีด : เลเวล +2", value="+2"),
    discord.SelectOption(label="🔪 มีด : เลเวล +3", value="+3"),
    discord.SelectOption(label="🔪 มีด : เลเวล +4", value="+4"),
    discord.SelectOption(label="🔪 มีด : เลเวล +5", value="+5"),
]

MACHETE_OPTIONS = [
    discord.SelectOption(label="❌ มาเชเต้ : ไม่มีอาวุธ", value="-"),
    discord.SelectOption(label="🗡️ มาเชเต้ : เลเวล +0", value="+0"),
    discord.SelectOption(label="🗡️ มาเชเต้ : เลเวล +1", value="+1"),
    discord.SelectOption(label="🗡️ มาเชเต้ : เลเวล +2", value="+2"),
    discord.SelectOption(label="🗡️ มาเชเต้ : เลเวล +3", value="+3"),
    discord.SelectOption(label="🗡️ มาเชเต้ : เลเวล +4", value="+4"),
    discord.SelectOption(label="🗡️ มาเชเต้ : เลเวล +5", value="+5"),
]

POOL_OPTIONS = [
    discord.SelectOption(label="❌ ไม้พูล : ไม่มีอาวุธ", value="-"),
    discord.SelectOption(label="🦯 ไม้พูล : เลเวล +0", value="+0"),
    discord.SelectOption(label="🦯 ไม้พูล : เลเวล +1", value="+1"),
    discord.SelectOption(label="🦯 ไม้พูล : เลเวล +2", value="+2"),
    discord.SelectOption(label="🦯 ไม้พูล : เลเวล +3", value="+3"),
    discord.SelectOption(label="🦯 ไม้พูล : เลเวล +4", value="+4"),
    discord.SelectOption(label="🦯 ไม้พูล : เลเวล +5", value="+5"),
]

GUN_OPTIONS = [
    discord.SelectOption(label="❌ ปืน : ไม่มีอาวุธ", value="-"),
    discord.SelectOption(label="🔫 ปืน : เลเวล +0", value="+0"),
    discord.SelectOption(label="🔫 ปืน : เลเวล +1", value="+1"),
    discord.SelectOption(label="🔫 ปืน : เลเวล +2", value="+2"),
    discord.SelectOption(label="🔫 ปืน : เลเวล +3", value="+3"),
    discord.SelectOption(label="🔫 ปืน : เลเวล +4", value="+4"),
    discord.SelectOption(label="🔫 ปืน : เลเวล +5", value="+5"),
]
...
class WeaponUpdateView(discord.ui.View):
    def __init__(self, user_id: int, slot_info: dict):
        ...
        # 🟢 [แก้ไข] ส่งตัวเลือกเฉพาะของอาวุธแต่ละชนิดเข้า Select
    ...
    async def confirm_btn(self, button: discord.ui.Button, interaction: discord.Interaction):
        # 🟢 [แก้ไข] ย้าย defer ขึ้นมาบรรทัดแรกสุด ป้องกัน Timeout 3 วินาที
        ...
        target_slot = None
        for s_key, s_val in weapons_data.items():
            if s_val.get("user_id") == str(self.user_id):
                target_slot = s_key
                break
                
        if not target_slot:
            followup_msg = await interaction.followup.send("⚠️ **ไม่พบข้อมูลสล็อตของคุณในระบบ!**", ephemeral=True)
            await asyncio.sleep(5)
        ...
        weapons_data[target_slot].update({"knife": knife_val, "machete": machete_val, "pool": pool_val, "gun": gun_val})
        save_weapons(gid, weapons_data)
        
        # 🟢 [แก้ไข] ครอบ try-except ล้อมการอัปเดตบอร์ด เพื่อความเสถียร
        try:
            await update_weapon_board(interaction.guild)
        except Exception as e:
            print(f"⚠️ Error updating weapon board: {e}")
        ...
        success_msg = await interaction.followup.send("✅ **อัปเดตเลเวลอาวุธของคุณเรียบร้อยแล้ว!**", ephemeral=True)
        await asyncio.sleep(5)
...
# --- 📌 ของเดิม / ส่วนที่ปรับปรุง: เมนูหลักจัดการสล็อต (เพิ่มปุ่มอัปเดตอาวุธแทน) ---
class AdminWeaponManageMainView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=300)

    @discord.ui.button(label="➕ เพิ่มรายชื่อเข้าสล็อต", style=discord.ButtonStyle.primary, row=0)
    async def add_member_btn(self, button: discord.ui.Button, interaction: discord.Interaction):
    ...
    # 🟢 [เพิ่มใหม่] ปุ่มสำหรับเลือกสล็อตแล้วอัปเดตอาวุธแทนสมาชิก
    @discord.ui.button(label="🔄 อัปเดตอาวุธแทน", style=discord.ButtonStyle.primary, row=0)
    async def update_member_gear_btn(self, button: discord.ui.Button, interaction: discord.Interaction):
        view = AdminSelectSlotForUpdateView(interaction.guild)
        if not view.children:
            return await interaction.response.send_message("⚠️ **ขณะนี้ไม่มีสมาชิกอยู่ในสล็อตใดเลย ไม่สามารถอัปเดตอาวุธได้ครับ!**", ephemeral=True)
        text = (
            "🔄 **อัปเดตอาวุธแทนสมาชิก**\n"
            "กรุณาเลือกสล็อตของสมาชิกที่ต้องการอัปเดตเลเวลอาวุธ:"
        )
        await interaction.response.edit_message(content=text, view=view)
...
# --- 🟢 [เพิ่มใหม่] หน้าเลือกสล็อตเพื่ออัปเดตอาวุธแทนสมาชิก ---
# --- 🟢 หน้าเลือกสล็อตเพื่ออัปเดตอาวุธ (แบ่งกลุ่ม 1-25 และ 26-30) ---
class AdminSelectSlotForUpdateView(discord.ui.View):
    def __init__(self, guild: discord.Guild):
        ...
        for i in range(1, 31):
            s_key = f"{i:02d}"
            s_key_alt = str(i)
            slot_info = weapons_data.get(s_key) or weapons_data.get(s_key_alt)
            if slot_info and slot_info.get("user_id"):
                raw_name = slot_info.get("name", "Unknown")
        ...
        # แบ่งกลุ่มตัวเลือกครั้งละ 25 รายการ (กลุ่มที่ 1: 1-25, กลุ่มที่ 2: 26-30)
        chunk_size = 25
        for idx, i in enumerate(range(0, len(options), chunk_size)):
            chunk = options[i:i + chunk_size]
            if not chunk: continue
            slot_select = discord.ui.Select(
                placeholder=f"🔍 เลือกสล็อตที่ต้องการอัปเดต (กลุ่มที่ {idx+1})...", 
                options=chunk, 
                row=idx
            )
            slot_select.callback = self.slot_cb
    ...
    @discord.ui.button(label="👉 ถัดไป (เลือกเลเวลอาวุธ)", style=discord.ButtonStyle.success, row=2)
    async def next_btn(self, button: discord.ui.Button, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        if not self.selected_slot:
            msg = await interaction.followup.send("⚠️ **กรุณาเลือกหมายเลขสล็อตก่อนครับ!**", ephemeral=True)
        ...
        gid = str(interaction.guild.id)
        weapons_data = load_weapons(gid)
        slot_info = weapons_data.get(self.selected_slot, {})
        target_user_id = slot_info.get("user_id")

        if not target_user_id:
            return await interaction.followup.send("⚠️ **ไม่พบข้อมูลสมาชิกในสล็อตนี้!**", ephemeral=True)

        view = AdminWeaponUpdateView(target_user_id=int(target_user_id), target_slot=self.selected_slot, slot_info=slot_info)
        await interaction.edit_original_response(content=f"🔄 **กำลังอัปเดตอาวุธให้สล็อตที่ {self.selected_slot}**", view=view)
...
class AdminWeaponUpdateView(discord.ui.View):
    ...
    async def confirm_btn(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        if self.target_slot not in weapons_data:
            followup_msg = await interaction.followup.send("⚠️ **ไม่พบข้อมูลสล็อตนี้ในระบบแล้ว!**", ephemeral=True)
            await asyncio.sleep(5)
            try: await followup_msg.delete()
            except: pass
            return
        ...
        success_msg = await interaction.followup.send(f"✅ **อัปเดตเลเวลอาวุธให้สล็อตที่ {self.target_slot} เรียบร้อยแล้ว!**", ephemeral=True)
        await asyncio.sleep(5)
        try: await success_msg.delete()
        except: pass

    @discord.ui.button(label="🔙 ย้อนกลับ", style=discord.ButtonStyle.secondary, row=4)
    async def back_btn(self, button: discord.ui.Button, interaction: discord.Interaction):
        view = AdminWeaponManageMainView()
        await interaction.response.edit_message(content="⚙️ **เลือกรายการที่ต้องการจัดการสล็อตอาวุธ:**", view=view)
...
class AdminAddMemberSlotView(discord.ui.View):
    ...
    async def save_btn(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        if not self.selected_slot:
            msg = await interaction.followup.send("⚠️ **กรุณาระบุหมายเลขสล็อตก่อนครับ!**", ephemeral=True)
            await asyncio.sleep(4)
            try: await msg.delete()
            except: pass
            return
        ...
        if not self.selected_user:
            msg = await interaction.followup.send("⚠️ **กรุณาเลือกสมาชิกที่ต้องการเพิ่มเข้าสล็อตก่อนครับ!**", ephemeral=True)
            await asyncio.sleep(4)
            try: await msg.delete()
            except: pass
            return
        ...
        # 🟢 [แก้ไขจุดที่ 2] แปลงเป็น Format ตัวเลข 2 หลัก (เช่น 05) เสมอ
        slot_key = f"{int(self.selected_slot):02d}"

        weapons_data[slot_key] = {
            "user_id": str(target_member.id),
            "name": target_member.display_name,
            "knife": "-", "machete": "-", "pool": "-", "gun": "-"
        }
        save_weapons(gid, weapons_data)

        # 🟢 [แก้ไขจุดที่ 3] ครอบ try-except ล้อมการอัปเดตบอร์ด ป้องกันปัญหาการประมวลผลค้าง
        try:
            await update_weapon_board(interaction.guild)
        except Exception as e:
            print(f"⚠️ Error updating board: {e}")
        ...
        if log_ch_id and log_ch_id != "disabled":
            try:
                log_ch = bot.get_channel(int(log_ch_id))
                if log_ch:
                    log_em = discord.Embed(title="📌 บันทึกการจัดการสล็อตอาวุธ (เพิ่มสมาชิก)", color=0xe67e22)
                    log_em.description = (f"**👮 ผู้ดำเนินการ:** {interaction.user.display_name} (Admin)\n"
                                          f"**🔢 สล็อต:** {slot_key}\n**👤 สมาชิก:** {target_member.display_name}\n\n{LONG_SEP}")
                    log_em.set_footer(text=f"บันทึกเมื่อ: {get_thai_time().strftime('%d/%m/%Y %H:%M:%S')}")
                    await log_ch.send(embed=log_em)
            except: pass
        ...
        success_msg = await interaction.followup.send(f"🟢 **เพิ่มสมาชิก `{target_member.display_name}` เข้าสล็อตที่ {slot_key} เรียบร้อยแล้ว!**", ephemeral=True)
        await asyncio.sleep(5)
        try: await success_msg.delete()
        except: pass
...
# --- 🟢 หน้าลบรายชื่อ (แบ่งกลุ่ม 1-25 และ 26-30) ---
class AdminRemoveMemberSlotView(discord.ui.View):
    def __init__(self, guild: discord.Guild):
        ...
        # แบ่งกลุ่มตัวเลือกครั้งละ 25 รายการ
        chunk_size = 25
        for idx, i in enumerate(range(0, len(options), chunk_size)):
            chunk = options[i:i + chunk_size]
            if not chunk: continue
            slot_select = discord.ui.Select(
                placeholder=f"🗑️ เลือกสล็อตที่ต้องการลบ (กลุ่มที่ {idx+1})...", 
                options=chunk, 
                row=idx
            )
            slot_select.callback = self.slot_cb
    ...
    @discord.ui.button(label="🔴 ยืนยันลบออกจากสล็อต (Full Wipe)", style=discord.ButtonStyle.danger, row=2)
    async def remove_btn(self, button: discord.ui.Button, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)

        if not self.selected_slot:
            msg = await interaction.followup.send("⚠️ **กรุณาเลือกหมายเลขสล็อตที่ต้องการลบก่อนครับ!**", ephemeral=True)
            await asyncio.sleep(4)
            try: await msg.delete()
            except: pass
            return
        ...
        s_key_padded = f"{int(self.selected_slot):02d}"
        s_key_raw = str(int(self.selected_slot))

        old_user_name = "Unknown"
        if s_key_padded in weapons_data:
            old_user_name = weapons_data[s_key_padded].get("name", "Unknown")
            weapons_data.pop(s_key_padded, None)
        if s_key_raw in weapons_data:
            old_user_name = weapons_data[s_key_raw].get("name", old_user_name)
            weapons_data.pop(s_key_raw, None)
        ...
        if log_ch_id and log_ch_id != "disabled":
            try:
                ...
                if log_ch:
                    ...
                    log_em.description = (f"**👮 ผู้ดำเนินการ:** {interaction.user.display_name} (Admin)\n"
                                          f"**🔢 ลบสล็อตที่:** {s_key_padded} (เดิมคือ: {old_user_name})\n\n{LONG_SEP}")
                    log_em.set_footer(text=f"บันทึกเมื่อ: {get_thai_time().strftime('%d/%m/%Y %H:%M:%S')}")
                    await log_ch.send(embed=log_em)
            except: pass
        ...
        success_msg = await interaction.followup.send(f"🗑️ **ล้างข้อมูลสล็อตและประวัติอาวุธที่ {s_key_padded} เรียบร้อยแล้ว (Full Wipe)!**", ephemeral=True)
        await asyncio.sleep(5)
        try: await success_msg.delete()
        except: pass
...
class WeaponSetupView(discord.ui.View):
    def __init__(self, guild: discord.Guild):
        ...
        self.log_select = discord.ui.Select(
            select_type=discord.ComponentType.channel_select,
            channel_types=[discord.ChannelType.text],
            placeholder="📋 พิมพ์ค้นหาหรือเลือกห้อง Audit Log...",
            row=1
        )
    ...
    async def log_cb(self, interaction: discord.Interaction):
    ...
    async def toggle_log_btn(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        if self.log_enabled:
            button.label = "📋 สถานะ Log: เปิดอยู่"
            button.style = discord.ButtonStyle.success
        else:
            button.label = "📋 สถานะ Log: ปิดอยู่"
...
# ==========================================
# --- 🟢 [แก้ไขจุดที่ 1] เมนูระบบอื่นๆ: ย้ายปุ่มปิดเมนูมาอยู่แถวเดียวกัน (row=0) และตัดปุ่มย้อนกลับออก ---
class OthersSystemView(discord.ui.View):
    ...
    async def weapon_system_setup(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        text = (
            "⚙️ **ตั้งค่าระบบอัปเดตอาวุธ**\n\n"
            "1. เลือกห้องสำหรับแสดงบอร์ดอาวุธ\n"
            "2. เลือกห้องบันทึกประวัติ (Audit Log)\n"
            "3. กดปุ่ม **'🟢 บันทึกการตั้งค่า'**"
        )
        await interaction.response.edit_message(content=text, view=view)

    # 🟢 [แก้ไข] ย้ายปุ่มปิดเมนูมาอยู่ row=0 แถวเดียวกับปุ่มตั้งค่า และลบปุ่มย้อนกลับออก
    @discord.ui.button(label="ปิดเมนู", style=discord.ButtonStyle.danger, row=0)
    async def close_btn(self, button: discord.ui.Button, interaction: discord.Interaction):
        await interaction.response.defer()
        try: await interaction.delete_original_response()
